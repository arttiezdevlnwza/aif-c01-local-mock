...
- **Adjacent retest sets must not reuse the same complete choice set question-for-question.** Compare choices order-insensitively because the UI shuffles them; simple A/B/C reordering does not count as a new set. If a concept must be retested, change the decision angle, scenario evidence, or adjacent distractor family rather than copying the previous choice set.
